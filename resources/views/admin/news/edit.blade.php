...
@section('content')
    ...
    <form action="{{ route($routePrefix . 'news.update', $news) }}" method="POST" enctype="multipart/form-data" id="newsForm">
        @csrf
        @method('PUT')
        ...
        <div style="display: grid; grid-template-columns: 1fr; gap: var(--space-xl); align-items: start; margin-top: var(--space-lg);">
            ...
            <div class="news-layout" style="display: grid; grid-template-columns: 1fr; gap: var(--space-xl);">
                ...
                <div class="main-content" style="display: flex; flex-direction: column; gap: var(--space-lg);">
                    <div class="admin-card" style="margin: 0;">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" class="form-input @error('title') is-invalid @enderror" value="{{ old('title', $news->title) }}" required placeholder="Masukkan judul yang menarik...">
                            @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="excerpt" id="excerpt" rows="2" class="form-textarea @error('excerpt') is-invalid @enderror" placeholder="Tuliskan intisari berita di sini...">{{ old('excerpt', $news->getAttributes()['excerpt'] ?? '') }}</textarea>
                            @error('excerpt')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom: 0;">
                            ...
                            <div id="editor-container" style="min-height: 500px; padding-bottom: 20px; font-size: 1.05rem;">{!! old('content', $news->content) !!}</div>
                            @error('content')
                ...
                <div class="sidebar" style="display: flex; flex-direction: column; gap: var(--space-lg);">
                    ...
                    <div class="admin-card" style="margin: 0; position: sticky; top: 20px;">
                        ...
                        <div class="form-group">
                            <label class="custom-switch" style="display: flex; align-items: center; gap: 10px; cursor: pointer; background: rgba(74, 124, 89, 0.05); padding: 12px; border-radius: var(--radius-sm);">
                                <input type="checkbox" name="is_published" value="1" {{ old('is_published', $news->is_published) ? 'checked' : '' }} style="width: auto;">
                                <span style="font-weight: 500;">Tayangkan (Publish)</span>
                            </label>
                        </div>
                    ...
                    <!-- Cover Image Card -->
                    <div class="admin-card" style="margin: 0;">
                        <h3 style="font-size: 1.1rem; margin-top: 0; margin-bottom: var(--space-md);">Foto Sampul</h3>
                        
                        <div class="form-group" style="margin-bottom: 0;">
                            <input type="file" name="image" id="image" class="form-input @error('image') is-invalid @enderror" accept="image/jpeg,image/png,image/webp" style="display: none;" onchange="previewImage(this, 'preview-image')">
                            
                            <div class="image-upload-wrap" onclick="document.getElementById('image').click()">
                                <div class="upload-placeholder" id="upload-placeholder" style="display: {{ $news->image_path ? 'none' : 'block' }};">
                                    <span class="material-symbols-outlined">add_photo_alternate</span>
                                    <p style="margin: 0; font-size: 0.9rem;">Klik untuk mengubah foto<br><small>(Biarkan kosong jika tidak ingin mengubah)</small></p>
                                </div>
                                <img id="preview-image" src="{{ $news->image_path ? asset('storage/' . $news->image_path) : '#' }}" alt="Preview" style="display: {{ $news->image_path ? 'block' : 'none' }};">
                            </div>
                    ...
                    <div class="admin-card" style="margin: 0;">
                        ...
                        <div class="form-group">
                            ...
                            <input name="tags" id="tags" class="form-input @error('tags') is-invalid @enderror" placeholder="Contoh: inovasi, lingkungan" value="{{ old('tags', $news->tags->pluck('name')->implode(',')) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="event_date" id="event_date" class="form-input @error('event_date') is-invalid @enderror" value="{{ old('event_date', $news->event_date ? $news->event_date->format('Y-m-d') : '') }}" required>
                            @error('event_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group" style="margin-bottom: 0;">
                            ...
                            <input type="text" name="location" id="location" class="form-input @error('location') is-invalid @enderror" value="{{ old('location', $news->location) }}" required placeholder="Contoh: Balai Desa Rowosari">
                            @error('location')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>

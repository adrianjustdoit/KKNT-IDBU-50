    ...
    <div class="admin-card">
        <form action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data" id="newsForm">
            ...
            <div class="form-group">
                ...
                <textarea name="excerpt" id="excerpt" rows="2" class="form-control @error('excerpt') is-invalid @enderror">{{ old('excerpt', $news->getAttributes()['excerpt'] ?? '') }}</textarea>
                @error('excerpt')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group" style="display: flex; gap: var(--space-xl); flex-wrap: wrap;">
                <div style="flex: 1; min-width: 250px;">
                    ...
                    <input type="date" name="event_date" id="event_date" class="form-control @error('event_date') is-invalid @enderror" value="{{ old('event_date', $news->event_date ? $news->event_date->format('Y-m-d') : '') }}" required>
                    @error('event_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div style="flex: 1; min-width: 250px;">
                    ...
                    <input type="text" name="location" id="location" class="form-control @error('location') is-invalid @enderror" value="{{ old('location', $news->location) }}" required>
                    @error('location')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <div class="mt-2">
                    <img id="preview-image" src="{{ $news->image_path ? asset('storage/' . $news->image_path) : '#' }}" alt="Preview" style="display: {{ $news->image_path ? 'block' : 'none' }}; max-width: 200px; border-radius: var(--radius-sm);">
                </div>
                @error('image')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
